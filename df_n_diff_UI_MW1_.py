from util import UI_MW, get_df_symbols_close, reindex_df
...
# ++++ get index dates from XOM.csv ++++
index_symbol_file = 'XOM.csv'
# dataframe df_symbols_close.txt exists, read it into df
df = pd.read_csv(path_symbols_data + index_symbol_file, index_col=0)
df.index.names = ['Date']  # change index name from '1962-01-02' to 'Date'
df = df[date_start:date_end]
print('{}.head():\n{}\n'.format(index_symbol_file, df.head()))
print('{}.tail():\n{}\n'.format(index_symbol_file, df.tail()))
# get index_symbol_file's(i.e. 'XOM') index dates
index_XOM = df.index
# print('index_XOM:\n{}\n'.format(index_XOM))
# print('type(index_XOM): {}\n'.format(type(index_XOM)))


# ++++ re-index df_symbols_close to dates in index_XOM ++++
#  BTC-USD has data every day. We only want BTC-USD data
#  on dated in index_XOM.
print("df_symbols_close shape before and after re-index with {}'s date:"
      .format(index_symbol_file))
# re-index df_symbols_close so all symbols have the same index
#  and delete Saturday price from BTC-USD
df_reindex = reindex_df(index_XOM, df_symbols_close)
print('before re-index shape: {};  after re-index shape: {}\n'
      .format(df_symbols_close.shape, df_reindex.shape))
df_symbols_close = df_reindex
...
print('Wrote df_diff_UI_MW_long.csv to {}\n'.format(path_data_dump))
# print('df_UI_MW_short.tail():\n{}\n'.format(df_UI_MW_short.tail()))
