# region program description (takes 1 second to run)
# The program removes rows with NaN and removes rows with CAGR_count less
# than CAGR_min_count from df_trade_results. It also adds four columns to the
# the df. The columns are: Rank_CAGR_mean/std, Rank_win_mean/std, Sum_Ranks,
# and Pct_Rank_Sum_Ranks. It saves the df to df_trade_results_ranked.
...
df = pd.read_pickle(path_data_dump + filename_df_in)
my_columns1 = [
    "file",
    "columns",
    "str_iloc_offsets",
...
# drop rows with CAGR_count less tha CAGR_min_count
CAGR_min_count = 20
print(
    f"len(df) before drop row with NaN and CAGR_count < {CAGR_min_count}: "
    f"{len(df)}"
)
# select rows using index is faster than query
index_drop = df[df["CAGR_count"] < CAGR_min_count].index
# Delete these row indexes from dataFrame
df.drop(index_drop, inplace=True)
df = df.dropna(subset=["CAGR_mean/std"])
print(
    f"len(df) after drop row with NaN and CAGR_count < {CAGR_min_count}: "
    f"{len(df)}"
)
...
df.to_pickle(path_data_dump + filename_df_out)
