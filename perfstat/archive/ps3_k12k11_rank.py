...
# str_iloc_offsets, slice_stop from df_ps1n2_results and save them to df_sym.
# It combines symbols in columns k12 and k11 to a new column k12_k11 in
...
mySymbols = ["QQQ", "ACH", "FTCH", "JWM", "NVCR", "SFIX", "MRNA"]

rows_OHLCV_display = 15  # number of rows of OHLCV to display
...
df = df[my_columns0].sort_values(["date_end"], ascending=False)
...
print(f"{my_date_end} unique {my_columns1[1]} symbol: {unique_symbols}\n")
...
df = pickle_load(path_data_dump, filename_df_ranked)
df = df.sort_values(by=["Pct_Rank_Sum_Ranks"], ascending=False)
print(f"{filename_df_ranked}.columns(len={len(df.columns)}):\n{df.columns}\n")
...
myFormat1 = (
    "{:<10s} {:>6s} {:>52s} {:>10d} {:>16d} {:>20s} {:>10.1f} {:>15.3f}"
)
cols = [
    "date_end",
    "symbol",
...
df_best_rank = pd.DataFrame(lst_dict, columns=cols)  # create df from lst_dict
...
# date_delay are NYSE trading dates. It's date_end delayed by days_trade_delay
...
# concatenate df_best_rank to df_best_rank in path_data_dump, and
# save and archive df_best_rank
df_concat = concat_df(
    df_best_rank,
    columns_drop_duplicate,
    file_df,
...
df_concat  = df_concat.sort_values(by=['date_end', 'symbol'])
pd.set_option("display.width", 400)
pd.set_option("display.max_columns", 10)
pd.set_option("display.max_colwidth", 15)
