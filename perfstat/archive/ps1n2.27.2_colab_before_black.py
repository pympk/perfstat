...
#  verbose = False
# v25 def ps1n2_2 was def ps1n2, append_df_2 was append_df,
...
# ########## Colab_change
# import sys
# sys.path.append('/content/drive/My Drive/python/py_files/')  # NOQA
# ########## Colab_change
from util import list_dump, get_symbol_data_2, append_df_2
from util import reindex_df, print_list_in_groups, pd_lists_intersection
from util import pickle_dump, pickle_load, dates_within_limits, list_sort
from util import symb_perf_stats_vectorized
# endregion


def ps1n2_2(date_end_limit, my_path, dfs, verbose=False):
    # v2 use append_df_2 was append_df
    #
    # region subset dfs to the largest index offset
    #  get XOM's index dates from '1962-01-2' to now
    date_index_all_dates = dfs[index_symbol].index

    if verbose:
        print('date_index_all_dates({}):\n{}\n'
              .format(type(date_index_all_dates), date_index_all_dates))

    # calculate start and end dates for iloc_offset=max_index_offset
    #  and their respective index locations
    date_start, date_end, iloc_date_start, iloc_date_end = \
        dates_within_limits(date_index_all_dates, date_start_limit,
                            date_end_limit, iloc_offset=max_index_offset,
                            verbose=verbose)

    # grab a smaller slice of date index that meets max_index_offset
    #  TypeError: date_index = date_index_all_dates[date_start:date_end]
    #  TypeError: slice indices must be integers or None or have an __index__
    date_index = date_index_all_dates[iloc_date_start:(iloc_date_end+1)]

    # re-index dataframes in dfs to XOM's index dates
    dfs_max_index_offset = {}  # working dfs
    if verbose:
        print('date_index({}):\n{}\n'.format(type(date_index), date_index))
        print('df shape before and after re-index')
        print(print_format2.format('symbol', 'df.shape', 'df_reindex.shape'))
        print('+'*78)
    # re-index df so all symbols have the same index
    #  dates without data are filled with NaN,
    #  dates not in index are dropped, e.g. delete Saturday price from BTC-USD
    for symbol in symbols:
        df = dfs[symbol]
        df_max_offset = reindex_df(date_index, df)
        # append symbol data to dictionary
        dfs_max_index_offset.update({symbol: df_max_offset})
        if verbose:
            print(print_format2
                  .format(symbol, str(df.shape), str(df_max_offset.shape)))
    if verbose:
        print('+'*78)
    # ++ dfs for further calculation ++
    dfs = dfs_max_index_offset
    # endregion subset dfs to the largest index offset
    ...
    for iloc_offset in iloc_offsets:
        ...
        if verbose:
            print('\n{}'.format('='*78))
            print('+ Calculate Perf. Stats: iloc_offset = {} days, Sorted by {}\n'  # NOQA
                  .format(iloc_offset, sort))
        ...
        date_start, date_end, iloc_date_start, iloc_date_end = \
            dates_within_limits(date_index, date_start_limit, date_end_limit,
                                iloc_offset, verbose=verbose)
        # convert dates from timestamp to string for
        #  def get_symbol_data_from_dir
        date_start = date_start.strftime('%Y-%m-%d')
        date_end = date_end.strftime('%Y-%m-%d')
        if verbose:
            print(print_format3.format('date_start_limit',
                                       str(date_start_limit)))
            print(print_format3.format('date_end_limit', str(date_end_limit)))
            print(print_format3.format('iloc_offset', str(iloc_offset)))
            print('date_start: {:<10s}, type: {}'
                  .format(date_start, type(date_start)))
            print('date_end:   {:<10s}, type: {}'
                  .format(str(date_end), type(date_end)))
            print('date_today: {:<10s}, type: {}\n'
                  .format(str(dt.date.today()), type(dt.date.today())))
        ...
        symbol_close_with_zeros = \
            df_close.columns[(df_close == 0).any()].tolist()
        # symbols with closing price NaN
        symbol_close_with_NaN = \
            df_close.columns[np.isnan(df_close).any()].tolist()
        list_dump(symbol_close_with_zeros, path_data_dump,
                  'symbol_close_with_zeros.txt')
        list_dump(symbol_close_with_NaN, path_data_dump,
                  'symbol_close_with_NaN.txt')

        df_volume = df_symbols_volume[date_start:date_end]
        symbol_volume_with_zeros = \
            df_volume.columns[(df_volume == 0).any()].tolist()
        symbol_volume_with_NaN = \
            df_volume.columns[np.isnan(df_volume).any()].tolist()
        list_dump(symbol_volume_with_zeros, path_data_dump,
                  'symbol_volume_with_zeros.txt')
        list_dump(symbol_volume_with_NaN, path_data_dump,
                  'symbol_volume_with_NaN.txt')
        if verbose:
            print('symbols close with zeros: {}'
                  .format(symbol_close_with_zeros))
            print('symbols close with Nan: {}'
                  .format(symbol_close_with_NaN))
            print('symbols volume with zeros: {}'
                  .format(symbol_volume_with_zeros))
            print('symbols volume with Nan: {}'
                  .format(symbol_volume_with_NaN))

        # data cleaning: list of columns/symbols to drop from df_close
        columns_drop = symbol_close_with_zeros + symbol_close_with_NaN + \
            symbol_volume_with_zeros + symbol_volume_with_NaN
        ...
        list_dump(columns_drop, path_data_dump,
                  'symbols_with_zeros_or_NaNs_dropped_from_perf_stat.txt')

        # calculate performance stats
        symbols_perf_stats, period_yr, drawdown, UI, max_drawdown, \
        ...
        for symbol in symbols_perf_stats:
            cache = (symbol, date_first, date_last, period_yr, CAGR[symbol],
                     UI[symbol], Std_UI[symbol], CAGR_Std[symbol],
                     CAGR_UI[symbol])
        ...
        # sort dataframe
        df_perf_stats_sorted = \
            df_perf_stats.sort_values(by=[sort], ascending=False)

        if verbose:
            print('df_perf_stats_sorted is sorted by {}\n'.format(sort))

            # write df_perf_stats_sorted to file
            df_perf_stats_sorted.to_pickle(path_data_dump +
                                           'df_perf_stats_sorted_' +
                                           str(iloc_offset))
        ...
        # if verbose:
            ...
            print('Top {}-{} symbols of df_perf_stats_sorted, sorted by "{}": \n'  # NOQA
                  .format(top_symbols_range.start, top_symbols_range.stop, sort))  # NOQA
            print('{}\n'.format(df_perf_stats_sorted[top_symbols_range]))
        # get a slice(top_symbols_range) of top symbols from sort
        symbols_sorted = \
            list(df_perf_stats_sorted.symbol[top_symbols_range].values)

        if verbose:
            # print top performing symbols for the iloc_offset duration
            #  in groups
            print('symbols in df_perf_stats_sorted_{} sorted by "{}", in groups of {}:'  # NOQA
                  .format(iloc_offset, sort, print_group_size))
            print_list_in_groups(symbols_sorted, print_group_size)
            print('symbols_sorted:\n{}'.format(symbols_sorted))
            print('iloc_offset: {}\n'.format(iloc_offset))
            print(f'- Calculate Perf. Stats: '
        ...
        if verbose:
            print('appended symbols in {} to top_symbols_iloc_offsets_runs'
                  .format(filename_pickle))
    ...
    sorted_unique_symbols, sorted_unique_counts, \
        sorted_symbol_n_count_tuples = \
        list_sort(top_symbols_iloc_offsets_runs, verbose=False)
    if verbose:
        print('sorted top_symbols_iloc_offsets_runs into unique symbols and counts, ')  # NOQA
    ...
    # symbol read from text file: file_symbols
    count_total_symbols = len(symbols)  # total number of symbols
    pct_unique_to_total = \
    ...
    symbol_count_keys_all = \
        symbol_count_keys_12_11_10 + symbol_count_keys_9_8_7 + \
        symbol_count_keys_6_5_4 + symbol_count_keys_3_2_1
    ...
    if verbose:
        ...
        print(print_format4
              .format('count brackets', 'no. of symbols', 'no. of symbols'))

        print('+'*78)
        print(print_format4
              .format('counts: 12, 11, 10',
        ...
        print('+'*78)
        print(print_format5
              .format('total_counts', str(symbol_count_keys_all)))
        print(print_format5.format('total_dfs_symbols',
                                   str(count_total_symbols)))
        print(print_format5.format('total_counts / total_dfs_symbols, %',
    ...
    if verbose:
        print('{} sorted unique symbol, count = all: \n{}'
              .format(top_symbols_iloc_offsets_runs, list_keys_all))
        print('{} sorted unique symbol count: \n{}\n'
              .format(top_symbols_iloc_offsets_runs, list_counts_all))

    # region print symbols in each count
    if verbose:
        print('Symbols in Each Count')
        print('+'*78)
        print('{} sorted unique symbol, count = 12: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_12))
        print('{} sorted unique symbol, count = 12: \n{}\n'
              .format('top_symbols_iloc_offsets_runs', list_counts_12))
        print('{} sorted unique symbol, count = 11: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_11))
        print('{} sorted unique symbol, count = 11: \n{}\n'
              .format('top_symbols_iloc_offsets_runs', list_counts_11))
        print('{} sorted unique symbol, count = 10: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_10))
        print('{} sorted unique symbol, count = 10: \n{}\n'
              .format('top_symbols_iloc_offsets_runs', list_counts_10))
        print('{} sorted unique symbol, count = 9: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_9))
        print('{} sorted unique symbol, count = 9: \n{}\n'
              .format('top_symbols_iloc_offsets_runs', list_counts_9))
        print('{} sorted unique symbol, count = 8: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_8))
        print('{} sorted unique symbol, count = 8: \n{}\n'
              .format('top_symbols_iloc_offsets_runs', list_counts_8))
        print('{} sorted unique symbol, count = 7: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_7))
        print('{} sorted unique symbol, count = 7: \n{}\n'
              .format('top_symbols_iloc_offsets_runs', list_counts_7))
        print('{} sorted unique symbol, count = 6: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_6))
        print('{} sorted unique symbol, count = 6: \n{}\n'
              .format('top_symbols_iloc_offsets_runs', list_counts_6))
        print('{} sorted unique symbol, count = 5: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_5))
        print('{} sorted unique symbol, count = 5: \n{}\n'
              .format('top_symbols_iloc_offsets_runs', list_counts_5))
        print('{} sorted unique symbol, count = 4: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_4))
        print('{} sorted unique symbol, count = 4: \n{}\n'
              .format('top_symbols_iloc_offsets_runs', list_counts_4))
        print('{} sorted unique symbol, count = 3: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_3))
        print('{} sorted unique symbol, count = 3: \n{}\n'
              .format('top_symbols_iloc_offsets_runs', list_counts_3))
        print('{} sorted unique symbol, count = 2: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_2))
        print('{} sorted unique symbol, count = 2: \n{}\n'
              .format('top_symbols_iloc_offsets_runs', list_counts_2))
        print('{} sorted unique symbol, count = 1: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_keys_1))
        print('{} sorted unique symbol, count = 1: \n{}'
              .format('top_symbols_iloc_offsets_runs', list_counts_1))
        print('+'*78)
    ...
    if verbose:
        print('sorted_symbol_n_count_tuples:\n{}\n'
              .format(sorted_symbol_n_count_tuples))
        print('- Accumulate and Flatten Lists of top symbols '
    ...
    with open(path_data_dump + filename_ps1n2_results, 'w') as f:
        ...
        f.write(print_format6.format('symb_uniq',
                                     str(count_total_unique_symbols)))
        f.write(print_format6.format('symb_lists',
                                     str(count_total_symbols_all_lists)))  # NOQA
        ...
        f.write(print_format6.format('pct_lists_all',
                                     str(pct_all_lists_to_total)))
        f.write(print_format6.format('iloc_offsets', str(iloc_offsets)))
        f.write(print_format6.format('top_symbols_range',
                                     str(top_symbols_range)))
        ...
        if verbose:
            ...
            print(print_format7.format('symb_uniq',
                                       str(count_total_unique_symbols)))
            print(print_format7.format('symb_lists', str(count_total_symbols_all_lists)))  # NOQA
            print(print_format7.format('symb_all', str(count_total_symbols)))
            print(print_format7.format('pct_uniq_all',
                                       str(pct_unique_to_total)))
            print(print_format7.format('pct_lists_all',
                                       str(pct_all_lists_to_total)))
            print(print_format7.format('iloc_offsets', str(iloc_offsets)))
            print(print_format7.format('top_symbols_range',
                                       str(top_symbols_range)))
            print(print_format7.format('path', str(my_path)))

    if verbose:
        print('\nTop {} symbols and counts in iloc_offsets are saved in file:'
              .format(sort))
    ...
    '{}':'{}'".format('date_end', date_end,
                      ...
                      'k2', list_keys_2,
                      'k1', list_keys_1,
                      'symb_uniq', count_total_unique_symbols,
                      'symb_lists', count_total_symbols_all_lists,  # NOQA
                      'symb_all', count_total_symbols,
                      'pct_uniq_all', pct_unique_to_total,
                      'pct_lists_all', pct_all_lists_to_total,
                      'str_iloc_offsets', str_iloc_offsets,
                      'slice_start', top_symbols_range.start,
                      'slice_stop', top_symbols_range.stop,
                      'path', my_path)

    print('top_symbols_range({}): {}'
          .format(type(str(top_symbols_range)), str(top_symbols_range)))
    ...
    df_run_result = \
        append_df_2(run_results, columns_drop_duplicate,
                    path_data_dump, filename_df,
                    path_archive, filename_df_archive, verbose)

    if verbose:
        print('df_run_result(shape = {}):\n{}\n'
              .format(df_run_result.shape, df_run_result))
    ...
    if verbose:
        print('---- date_end_limit {} loop finished on ----: {}'
              .format(date_end_limit, dt.datetime.now()))
...
my_path = 'C:/Users/ping/Google Drive/stocks/MktCap2b_AUMtop1200/'  # NOQA
# my_path = '/content/drive/My Drive/stocks/MktCap2b_AUMtop1200/'
# ########## Colab_change
...
file_symbols = path_symbols_file + 'MktCap2b_AUMtop1200.txt'

# date_end_limit is the last date used in performance stats calculation
# date_end_limit = '2020-09-04'  # test
# date_end_limit = '2013-02-06'  # next colab run
# date_end_limit = '2020-12-02'
date_end_limit = dt.date.today().strftime('%Y-%m-%d')

# ++++ takes 9 hours to run 100 iloc_iteration ++++
# iloc_iteration is number of iteration loop for performance stats
#  took 19 hours for 50 iterations
# iloc_iteration = 200  # iloc_iteration = 0 results in 1 iteration
iloc_iteration = 11  # iloc_iteration = 0 results in 1 iteration, 1.3 hrs for 5
...
# set_iloc_offsets = [iloc_offsets10]
set_iloc_offsets = [iloc_offsets1, iloc_offsets2, iloc_offsets3,
                    iloc_offsets4, iloc_offsets5, iloc_offsets6,
                    iloc_offsets7, iloc_offsets8, iloc_offsets9,
                    iloc_offsets10]
...
set_top_symbols_ranges = [top_symbols_range1, top_symbols_range2,
                          top_symbols_range3, top_symbols_range4]
...
# filename_df_csv = 'df_ps1n2_results.csv'
...
columns_drop_duplicate = ['date_end', 'str_iloc_offsets', 'slice_start',
                          'slice_stop']
...
df = pd.read_csv(path_symbols_data + index_symbol + '.csv', names=col_names,
...
if date_start_limit is not None:
    if date_start_limit not in date_index_all_dates:
        ...
        raise (ValueError(error_msg_date_not_in_index
               .format(date_start_limit, date_index_all_dates)))
...
if date_end_limit is not None:
    if date_end_limit not in date_index_all_dates:
        ...
        raise (ValueError(error_msg_date_not_in_index
               .format(date_end_limit, date_index_all_dates)))
# endregion error check dates

# region read symbols' OHLCV .csv files from directory
# Is OHLCV data current?
try:  # does file exists?
    df_tmp = pd.read_pickle(path_data_dump + 'df_symbols_close')
except FileNotFoundError:
    print(f"No such file or directory: {path_data_dump + 'df_symbols_close'}")
    print("Getting symbols' OHLCV data")
    dfs, symbols_with_csv_data, symbols_no_csv_data, \
        df_symbols_close, df_symbols_volume = \
        get_symbol_data_2(dir_path=path_symbols_data,
                          file_symbols=file_symbols,
                          date_start=None, date_end=None,
                          index_symbol=index_symbol, verbose=verbose)
    symbols = symbols_with_csv_data
    pickle_dump(dfs, path_data_dump, 'dfs_symbols_OHLCV_download')
    pickle_dump(symbols_with_csv_data, path_data_dump, 'symbols_with_csv_data')
    ...
    list_dump(symbols_with_csv_data, path_data_dump,
              'symbols_with_csv_data.txt')
    ...
    df_tmp = df_symbols_close

date_index_last = df_tmp.index[-1].strftime('%Y-%m-%d')
# release df from memory
df_tmp = pd.DataFrame()
print(f'date_index_last: {date_index_last}, date_end_limit: {date_end_limit}')
# date_end_limit is newer than df_symbols_close's last date
#  update symbols' OHLCv data
if date_index_last < date_end_limit:  # need to update OHLCV data
    print("date_end_limit is newer than df_symbols_close's "
          "last date, updating OHLCV data\n")
    # read symbols' OHLCV .csv files from directory
    #  dfs is a dictionary with dataframes of symbols' OHLCV data
    #  symbols is list of symbols, e.g. ['AMZN', 'QQQ', 'SPY', ...]
    #  df_symbols_close has index_symbol's date index,
    #   symbols' closing prices in columns with their symbol as column names
    #   symbols with dates that are not in date index, closing price for
    #    those dates are dropped.
    #   symbols with missing dates, closing price for those dates are filled
    #    with NaN when called
    #  df_symbols_volume is similar to df_symbols_close,
    #   except symbols' prices are replaced by symbols' volumes
    dfs, symbols_with_csv_data, symbols_no_csv_data, \
        df_symbols_close, df_symbols_volume = \
        get_symbol_data_2(dir_path=path_symbols_data,
                          file_symbols=file_symbols,
    ...
    pickle_dump(symbols, path_data_dump, 'symbols')
    pickle_dump(symbols_no_csv_data, path_data_dump, 'symbols_no_csv_data')
    # pickle_dump(df_symbols_close, path_data_dump, 'df_symbols_close')
    # pickle_dump(df_symbols_volume, path_data_dump, 'df_symbols_volume')
    df_symbols_close.to_pickle(path_data_dump + 'df_symbols_close')
    df_symbols_volume.to_pickle(path_data_dump + 'df_symbols_volume')
    list_dump(symbols_with_csv_data, path_data_dump,
              'symbols_with_csv_data.txt')
    ...
    print("date_end_limit is equal or older than df_symbols_close's, "
          "last date, retrieving OHLCV data\n")
    symbols = pickle_load(path_data_dump, 'symbols')
    symbols_with_csv_data = symbols
    dfs = pickle_load(path_data_dump, 'dfs_symbols_OHLCV_download')
    symbols_no_csv_data = pickle_load(path_data_dump, 'symbols_no_csv_data')
    df_symbols_close = pd.read_pickle(path_data_dump + 'df_symbols_close')
    df_symbols_volume = pd.read_pickle(path_data_dump + 'df_symbols_volume')

# print index, symbol
if verbose:
    print('symbols list')
    print('{:<7s}  :  {:10s}  '.format('index', 'symbol'))
    for i, symbol in enumerate(symbols):  # create index for list
        print('{:<7d}  :  {:10s}  '.format(i, symbol))
# endregion read symbols' OHLCV .csv files from directory

# region get a list of end_dates to calculate performance stats
# get iloc of date_end_limit in XOM's date index
date_start, date_end, iloc_date_start, iloc_date_end = \
    dates_within_limits(date_index_all_dates, date_start_limit, date_end_limit,
                        iloc_offset=0, verbose=verbose)
...
if verbose:
    print('date_end_limit({}): {}\n'.format(type(date_end_limit),
                                            date_end_limit))
...
open(path_data_dump + "_progress_ps1n2.25.txt", "w")
# iteration order: end_dates, set_top_symbols_ranges, set_iloc_offsets
for iloc_offsets, top_symbols_range in itertools\
        .product(set_iloc_offsets, set_top_symbols_ranges):
    # convert iloc_offsets to str for subsetting dataframe df_ps1n2_results,
    #  e.g. [18, 36] to '[18, 36]'
    str_iloc_offsets = "'" + str(iloc_offsets) + "'"

    if verbose:
        print('str_iloc_offsets({}): {}'
              .format(type(str_iloc_offsets), str_iloc_offsets))
    ...
    for i, end_date in enumerate(end_dates[::-1], start=-len(end_dates)):
        if verbose:
            print('+-='*40)
            print(f'START: i: {i}, end_date: {end_date}, '
                  f'iloc_offsets: {iloc_offsets}')
            print('+-='*40, '\n')
        # calculate performance stats of the symbols
        ps1n2_2(end_date, my_path, dfs)
        if verbose:
            print('+-='*40)
            print(f'END: i: {i}, end_date: {end_date}, '
                  f'iloc_offsets: {iloc_offsets}')
            print('+-='*40, '\n')

        # output file to check running process
        with open(path_data_dump + "_progress_ps1n2.25.txt", "a") as outfile:
            date_time = dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            print(f'counter: {counter},  date_time: {date_time}\n'
                  f'iloc_offsets: {iloc_offsets}\n'
...
df_temp = pd.read_csv(path_symbols_file + 'MktCap2b.csv',
...
df_temp = \
    pd.read_csv(path_symbols_file + 'AUMtop1200.csv',
                encoding='unicode_escape')
...
columns_ps1n2 = ['k1', 'k2', 'k3', 'k4', 'k5', 'k6', 'k7', 'k8',
                 'k9', 'k10', 'k11', 'k12']
df_ps1n2_results_stock[columns_ps1n2] = \
    df_ps1n2_results_stock[columns_ps1n2]\
    .apply(pd_lists_intersection, a_list=all_stock_symbols, verbose=verbose)
df_ps1n2_results_etf[columns_ps1n2] = \
    df_ps1n2_results_etf[columns_ps1n2]\
...
print('Wrote df_ps1n2_results_stock to:\n   {}{}'
      .format(path_data_dump, 'df_ps1n2_results_stock.csv'))
...
print('Wrote df_ps1n2_results_etf to:\n {}{}'
      .format(path_data_dump, 'df_ps1n2_results_etf.csv'))
