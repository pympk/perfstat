...
def ps1n2_2(date_end_limit, my_path, dfs, verbose=False):
    ...
    for symbol in symbols:
        ...
        if verbose:
            print(
                print_format2.format(
                    symbol, str(df.shape), str(df_max_offset.shape)
                )
            )
    if verbose:
        print("+" * 78)
    ...
    for iloc_offset in iloc_offsets:
        ...
        if verbose:
            print(
                "appended symbols in {} to top_symbols_iloc_offsets_runs".format(
                    filename_pickle
                )
            )
    ...
    if verbose:
        print(
            "sorted top_symbols_iloc_offsets_runs into unique symbols and counts, "
        )  # NOQA
    ...
    if verbose:
        print(
            "\nTop {} symbols and counts in iloc_offsets are saved in file:".format(
                sort
            )
        )
    ...
    if verbose:
        print(
            "---- date_end_limit {} loop finished on ----: {}".format(
                date_end_limit, dt.datetime.now()
            )
        )
...
try:  # does file exists?
    ...
    list_dump(
        symbols_with_csv_data, path_data_dump, "symbols_with_csv_data.txt"
    )
...
if date_index_last < date_end_limit:  # need to update OHLCV data
    print(
        "date_end_limit is newer than df_symbols_close's "
    ...
    print(
        "date_end_limit is equal or older than df_symbols_close's, "
...
df_temp = pd.read_csv(
    path_symbols_file + "AUMtop1200.csv", encoding="unicode_escape"
)
