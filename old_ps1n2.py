...
from get_symbol_data_3 import get_symbol_data_3
...
def calc_perf_stats():
    ...
    count_total_symbols = len(symbols_with_csv_data)  # total number of symbols
    #######################
...
iloc_iteration = 5  # iloc_iteration = 0 results in 1 iteration, 1.3 hrs for 5
...
if force_update_df_symbols_close:  # force update
    ...
    if (
        ...
        symbols_with_csv_data = pickle_load(
            path_data_dump, "symbols_with_csv_data"
        )
...
if verbose:
    ...
    for i, symbol in enumerate(symbols_with_csv_data):  # create index for list
...
open(path_data_dump + "_progress_df_ps1n2_results.txt", "w")
