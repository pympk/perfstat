# region program description (takes 12 hours 40 minutes to run)
...
#    check need to reload file_df_results
...
file_df_results_current = ""  # check need for re-load file_df_results

# output file to check running process
file_progress_monitor = "_ps3_df_trade_results.txt"
# open will truncate the file to zero length
open(path_data_dump + file_progress_monitor, "w")

counter = 0  # progress counter for loop iterations
# iteration order: cols, iloc_offsets, days_trade_delay, slice_stops
for (
    file_df_results,
    slice_stop,
    days_trade_delay,
    ...
    if file_df_results != file_df_results_current:  # reload if not current
        df_org = pickle_load(path_data_dump, file_df_results)
        # check need to reload file_df_results
        file_df_results_current = file_df_results
        print(
            f"file_df_results: {file_df_results}, "
            f"file_df_results_current: {file_df_results_current}"
        )
    ...
    # output file to check running process
    with open(path_data_dump + file_progress_monitor, "a") as f:
